...
public class index extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request,
		...
		if (request.getParameter("newpage") == null) {
			...
			path = request.getParameter("newpage").replaceAll("\\s+", "_");
		}
		if (path.equals("") || path.trim().equals("_"))
			path = "index";
		...
		try {
			...
			revisionId = -1; // Show the latest revision if the parameter
								// revision is not found
		}

		// In here findout if the page exits if it does show the latest revision
		// or the which one that the revision variable is.
		...
		try {
			...
			if (flag) {
				...
				// Connection c2 = DriverManager.getConnection( url, username,
				// password );
				...
				PreparedStatement pstmt2 = c.getConnected().prepareStatement(
						sql2);
				...
				Revision revisionEntry = new Revision(); // This is used to
															// display the
															// content of the
															// revision
				...
				} else // Get the revision specified
				{
					...
					PreparedStatement pstmt3 = c.getConnected()
				...
				request.getRequestDispatcher(
						"/WEB-INF/wiki/ShowContentPage.jsp").forward(request,
						response);
				...
				request.getRequestDispatcher(
						"/WEB-INF/wiki/PageDoesNotExitPage.jsp").forward(
						request, response);
